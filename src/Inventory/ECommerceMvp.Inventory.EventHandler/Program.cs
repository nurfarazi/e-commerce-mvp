...
using ECommerceMvp.Inventory.EventHandler;
...
var host = Host.CreateDefaultBuilder(args)
    ...
    .ConfigureServices((context, services) =>
    {
        ...
        // Inventory services
        services.AddSingleton<IInventoryProjectionWriter>(provider =>
        {
            var mongoClient = provider.GetRequiredService<IMongoClient>();
            var mongoOptions = provider.GetRequiredService<MongoDbOptions>();
            var logger = provider.GetRequiredService<ILogger<InventoryProjectionWriter>>();
            return new InventoryProjectionWriter(mongoClient, mongoOptions.DatabaseName, logger);
        });

        // Worker
        services.AddHostedService<InventoryEventWorker>();
    })
    .Build();
